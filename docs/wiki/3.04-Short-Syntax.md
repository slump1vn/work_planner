...
Work Planner supports four main short syntax forms for quickly adding metadata to tasks: tags (`#`), projects (`+`), time estimates (`m`/`h`/`d`), and due dates (`@`). Each form can be enabled or disabled individually in [[3.02-Settings-and-Preferences#global-settings.Short-Syntax]].
